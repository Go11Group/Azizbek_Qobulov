...
func main() {
	...
	userInfo, productInfo := data.InsertData()
	for _, u := range userInfo {
		if err := uRepo.CreateUser(u); err != nil {
			fmt.Println("Error creating user:", err)
			continue
		}
	}
	for _, p := range productInfo {
		if err := pRepo.CreateProduct(p); err != nil {
	...
	products, err := pRepo.GetProduct(model.Product{})
	if err != nil {
	...
	for _, u := range users {
		...
		for i := 0; i < numProducts; i++ {
			...
			err := upRepo.AddProduct(u.ID, randProduct.ID, qu)
			if err != nil {
...
func printUsers(upRepo *postgres.UserProductRepo, users []model.User) {
	for _, u := range users {
		...
		for _, p := range products {
			fmt.Printf("- %s: %s (Quantity: %d)\n", p.Name, p.Description, p.Quantity)
		}
	}
}
